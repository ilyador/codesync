...
export function useTaskEditorState() {
  ...
  const startEditingTask = (task: TaskView, rawTask?: TaskRecord) => {
    setEditingTask({
      ...
      provider_config_id: rawTask?.provider_config_id ?? null,
      provider_model: rawTask?.provider_model ?? null,
      execution_settings_locked_at: rawTask?.execution_settings_locked_at ?? null,
      images: task.images,
