  ...
  while (i < steps.length) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      try {
        ...
        // Persist to DB after each phase so progress survives crashes
        ...
        if (attempt >= maxAttempts) {
          ...
          try {
            ...
            // Checkpoint revert failed -- force-clean the worktree
            try {
              const { resetWorktree } = await import('./worktree.js');
              resetWorktree(localPath);
              onLog('[worktree] Force-cleaned dirty changes after failure\n');
              failMessage += '. Changes have been automatically reverted.';
            } catch {
