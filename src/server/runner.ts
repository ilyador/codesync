...
export interface RunnerTask extends Record<string, unknown> {
  id: string;
  title: string;
...
export async function scanAndUploadArtifacts(
  ...
  for (const filename of files) {
    ...
    try {
      ...
      hadFailure = true;
      onLog(`[artifact] Failed to capture ${filename}: ${errorMessage(err)}\n`);
    }
  }
  if (hadFailure) {
...
export interface FlowJobContext {
  ...
  task: RunnerTask;
  flow: FlowConfig;
  phasesAlreadyCompleted: PhaseRunRecord[];
  onLog: (text: string) => void;
  onPhaseStart: (phase: string, attempt: number) => void;
  onPhaseComplete: (phase: string, output: PhaseRunRecord) => void;
  onPause: (question: string) => void;
  onReview: (result: RunnerReviewResult) => Promise<void> | void;
...
export async function buildStepPrompt(
  ...
  if (flow.provider_binding === 'task_selected' && task.multiagent === 'yes') {
...
export async function runFlowJob(ctx: FlowJobContext): Promise<void> {
  ...
  const phasesCompleted: PhaseRunRecord[] = [...phasesAlreadyCompleted];
  const completedPhaseNames = new Set(phasesAlreadyCompleted.map(p => p.phase));
  ...
  while (i < steps.length) {
    ...
    for (let attempt = 1; attempt <= maxAttempts - attemptOffset; attempt++) {
      ...
      try {
        ...
        const { parsed, config } = await resolveProviderConfig(ctx.projectId, step.model, step.provider_config_id);
        ...
        const output = await driver.run({
          ...
          cwd: localPath,
          tools: step.tools,
          effort: flow.provider_binding === 'task_selected' && typeof task.effort === 'string'
            ? toProviderReasoningLevel(parsed.provider, task.effort as 'low' | 'medium' | 'high' | 'max')
            : undefined,
        ...
        onLog(`\nError in step ${step.name}: ${errorMessage(err)}\n`);
        if (displayAttempt >= maxAttempts) {
          if (!await isJobStillRunning(jobId)) return;
          let failMessage = `Step '${step.name}' failed: ${errorMessage(err)}`;
          try {
            ...
            onLog(`[checkpoint] Could not revert: ${errorMessage(revertErr)}\n`);
  ...
  try {
    const phaseLog = phasesCompleted.map((p) => {
    ...
    finalSummary = await generateFlowSummary(
      jobId,
      ctx.projectId,
      summaryPrompt,
      localPath,
    ...
    if (errorMessage(err) === 'Job canceled') return;
    console.error('[runner] Summary generation failed:', errorMessage(err));
    finalSummary = `Completed (summary unavailable: ${errorMessage(err)})`;
