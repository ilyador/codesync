...
## 2026-04-12 — Project job view models (`src/web/lib/project-job-view-models.ts`)

### Scope

Review of the 119-line view-model module that transforms `JobRecord[]` into `JobView[]` for UI rendering. Read the 64-line test file for coverage baseline. No subagent — small enough to review inline.

### Module shape

- `cleanSummary(raw)` — strips empty and `[`-prefixed lines from review summaries.
- `phaseList(...)` — ordered unique phase names from flow snapshot or completed-phases.
- `buildPhases(...)` — maps phase names to `{ name, status: completed|current|pending, summary? }`.
- `buildJobViews(jobs, taskTitleMap)` — main export: sorts by status priority, maps to `JobView`.
- `buildPrimaryJobViews(views)` — delegates to `pickPrimaryJobs`.

### Findings

| # | Severity | File | Status |
|---|---|---|---|
| 1 | LOW | `project-job-view-models.test.ts` — only phase derivation tested; sorting, review mapping, cleanSummary, missing-title, testsPassed semantics all uncovered | **Fixed** (a89f9e0) |

### Fix in this pass

**a89f9e0 — 6 tests covering the untested paths.** Test count 333 to 339.

- Status-priority sort order: running > queued > review > done > failed.
- review_result mapping: snake_case fields, cleanSummary stripping `[bracket]` lines, all numeric defaults (0 for filesChanged/linesAdded/linesRemoved).
- testsPassed intentionally left undefined when neither `tests_passed` nor `testsPassed` field is present — pins the `boolean | undefined` semantics the UI relies on (`TaskCardActiveDetail.tsx:144` uses `=== true` strict comparison to mean "tests ran and passed" vs. "unknown").
- Missing-title fallback to "Task"; empty jobs array; null review_result producing undefined review field.

### Verified-correct design decisions

- **`testsPassed` has no `?? 0` default.** Superficially inconsistent with `filesChanged ?? 0` etc., but intentional: it's `boolean | undefined`, not a number. `undefined` = unknown, `true` = tests passed, `false` = tests failed. New test pins this.
- **`cleanSummary` filtering `[`-prefixed lines.** Strips runtime status lines (`[done] Phase complete`, `[codex] tokens: ...`) from the review summary. Slightly aggressive if AI legitimately writes `[text]` — but review summaries are generated by the orchestrator, not freeform user content.
- **Status sort via hardcoded order map with `?? 5` fallback.** Unknown statuses sort to the end (same as `failed`). Stable sort preserves relative order within the same status.
